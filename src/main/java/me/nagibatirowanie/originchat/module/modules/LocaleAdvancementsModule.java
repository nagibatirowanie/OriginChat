 ...
 */
 ...
 public class LocaleAdvancementsModule extends AbstractModule implements Listener {
     ...
     public void onPlayerAdvancementDone(PlayerAdvancementDoneEvent event) {
         ...
         // Получаем игрока, который получил достижение
         Player achiever = event.getPlayer();
         Component titleComponent = advancement.getDisplay().displayName();
         
         // Отправляем сообщение ВСЕМ игрокам на сервере в их локали
         for (Player recipient : Bukkit.getOnlinePlayers()) {
             String recipientLocale = recipient.getLocale().toLowerCase(Locale.ROOT);
             String format = localeFormats.getOrDefault(recipientLocale, defaultFormat);
             
             if (format.isEmpty()) {
                 debug("Format for locale " + recipientLocale + " empty for player " + recipient.getName());
                 continue;
             }
             
             Component message = buildAdvancementMessage(format, achiever, titleComponent);
             recipient.sendMessage(message);
         }
         
         debug("Custom achievement message sent to all players for achievement by " + achiever.getName());
     }
